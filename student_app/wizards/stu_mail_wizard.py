...
class StudentMailWizard(models.TransientModel):
    ...
    email_data_wizard = fields.Html('Data of student')
    ...
    def Send_mail_user(self):
        ...
        for rec in self:
            for stu in rec.mail_to:
                ...
                # jsu remember method serch/ rec.mail_to = self.env['student.data'].browse([stu.id]).email

                email_values = {'email_to': self.env['student.data'].browse([stu.id]).email, 'subject': self.subject_msg, 'attachment_ids': [(4, dt.id) for dt in self.attechment_ids]
                                }
                print("\n\n\n\n mail\n\n\n\n",
                    email_values)
